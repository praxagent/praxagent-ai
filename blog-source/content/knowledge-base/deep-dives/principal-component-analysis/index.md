...
{{< reference-figure src="knowledge-base/deep-dives/pca-first-words.svg" label="Variation and correlation before PCA" alt="A two-part diagram compares root-length values with the same mean but low or high variance, then shows positive, negative, and near-zero patterns of linear correlation." caption="Left: both root-length rows average 27 millimeters, but the wider row has greater variance because its values lie farther from the mean. Right: the three point clouds show positive, negative, and near-zero linear correlation. Correlation describes a tendency, not a causal relationship. These standalone teaching values are not part of the wheat-kernel dataset analyzed later." >}}
