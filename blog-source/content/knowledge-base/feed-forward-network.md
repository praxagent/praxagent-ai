...
In [Transformer]({{< relref "transformer.md" >}}) writing, **FFN** usually means something narrower: the **position-wise feed-forward sublayer** inside each block. The same learned function transforms every token-position state separately. A [multilayer perceptron (MLP)]({{< relref "mlp.md" >}}) is a common implementation of that function.

{{< reference-figure
  src="knowledge-base/glossary/feed-forward-network-positionwise.svg"
...
- The FFN's output is an internal update to the [residual stream]({{< relref "residual-stream.md" >}}), not a final token probability.

See also: [multilayer perceptron (MLP)]({{< relref "mlp.md" >}}), [Transformer]({{< relref "transformer.md" >}}), [residual stream]({{< relref "residual-stream.md" >}}).
