...
**Temperature** is a positive scalar applied to next-token logits before they are turned into probabilities for [sampling]({{< relref "knowledge-base/glossary/sampling/index.md" >}}):
...
For every positive \(T\), division by \(T\) preserves logit order. A [greedy continuation]({{< relref "knowledge-base/glossary/greedy-continuation/index.md" >}}) therefore chooses the same argmax even though the inspectable probability distribution changes. API settings called `temperature: 0` are normally a special convention for greedy or near-greedy decoding, not the formula above evaluated at zero.

Temperature and [top-p]({{< relref "knowledge-base/glossary/top-p/index.md" >}}) interact: temperature changes which tokens accumulate the nucleus mass, so record both settings and the order in which an inference library applies them.

See also: [top-p]({{< relref "knowledge-base/glossary/top-p/index.md" >}}), [sampling]({{< relref "knowledge-base/glossary/sampling/index.md" >}}), [unembedding]({{< relref "knowledge-base/glossary/unembedding/index.md" >}}).
