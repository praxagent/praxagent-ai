...
The **logit lens** (also called the *identity* transport in our notes) takes one mid-layer [residual-stream]({{< relref "knowledge-base/glossary/residual-stream/index.md" >}}) vector and applies the model's final readout *as if that vector were already in final-layer coordinates*. A typical implementation is

\[
z_{\ell,t} = W_U\,N_f(h_{\ell,t}) + b_U,
\]

where \(N_f\) is the model's final normalization and \(W_U\) and \(b_U\) are its [unembedding]({{< relref "knowledge-base/glossary/unembedding/index.md" >}}) parameters. Architectural details differ: the correct normalization, bias, and tied-weight convention must match the model being inspected. Skipping the final normalization can materially change the ranking.
...
Conversely, a difference between the identity readout and a [Jacobian lens]({{< relref "knowledge-base/glossary/jacobian-lens/index.md" >}}) shows sensitivity to the transport. It does not, without matched prompts and interventions, establish that either lens has recovered the model's mechanism.

See also: [base-rate bias]({{< relref "knowledge-base/glossary/base-rate-bias/index.md" >}}), [Jacobian lens]({{< relref "knowledge-base/glossary/jacobian-lens/index.md" >}}), [random-J]({{< relref "knowledge-base/glossary/random-j/index.md" >}}), [best-rank]({{< relref "knowledge-base/glossary/best-rank/index.md" >}}).
