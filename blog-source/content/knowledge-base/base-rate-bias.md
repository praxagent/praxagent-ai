...
Suppose lower [rank]({{< relref "rank.md" >}}) is stronger and the same tested readout uses the same layer-by-position search in both prompt conditions:
...
1. **Intermediate-state readout.** A [logit lens]({{< relref "logit-lens.md" >}}) applies the model's final normalization and unembedding directly to an intermediate residual state, implicitly treating that state as readable in final-layer coordinates.
...
- Score the tested [Jacobian lens]({{< relref "jacobian-lens.md" >}}), [logit lens]({{< relref "logit-lens.md" >}}), and [random-J]({{< relref "random-j.md" >}}) on the same prompts, token variants, layers, and positions.
