...
{{< reference-figure
  src="knowledge-base/glossary/rms-size-not-direction.svg"
...
In these notes, **residual RMS** means \(\operatorname{RMS}(h)\) for one chosen [residual-stream]({{< relref "residual-stream.md" >}}) vector, normally at a specified layer and token position. An intervention protocol may state a signed amplitude \(\alpha\) relative to that scale. For a unit-RMS direction \(u\), the requested edit is
...
See also: [residual stream]({{< relref "residual-stream.md" >}}), [Transformer]({{< relref "transformer.md" >}}), [Jacobian lens]({{< relref "jacobian-lens.md" >}}).
