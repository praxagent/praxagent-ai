...
- For the paired [Wilcoxon signed-rank test]({{< relref "wilcoxon.md" >}}), the exact conditional calculation requires the joint distribution of the nonzero differences to be invariant under independently flipping any subset of their signs. Independent differences that are each symmetric around zero are a standard sufficient condition; marginal symmetry alone is not enough.
