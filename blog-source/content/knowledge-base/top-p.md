...
Like temperature, top-p does not change an ordinary [greedy continuation]({{< relref "greedy-continuation.md" >}}) when the maximum is unique. With tied maxima, applying top-p before argmax also requires compatible sorting and tie-breaking rules; an unfiltered greedy decoder simply ignores top-p.
