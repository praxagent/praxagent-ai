...
lead: |
  ...
  This note is the postmortem. It shows the attractive wrong result, the failed repair, the corrected experiment, and the narrower claim that survived: on Qwen, the matched self-vs-other-model contrast moves in the predicted direction on **14/16 wordings, [p=0.004](/blog/references/p-value/)**. A follow-up can focus on what that finding means. Here the subject is how small design details changed the scientific claim.
draft: "false"
---
...
- **Frozen paraphrase tests.** Each construct uses matched wording pairs frozen before outcomes. We report the median {{< refterm "rank" "rank" >}} in each arm, the number of pairs moving in the predicted direction, and a [sign test](/blog/references/p-value/) plus {{< refterm "wilcoxon" "Wilcoxon" >}} across them. Freezing prevents outcome-driven rewriting, but it does not make a bad comparison valid. That is the central lesson of Round two.

Technical terms link to [References](/blog/references/) on first use (and whenever a definition helps). Teaching detail for the instrument lives in the [Jacobian-lens release](../praxagent-jacobian-lens-qwen3-5-397b-a17b/). For when a {{< refterm "jacobian-lens" "Jacobian lens" >}} earns its keep versus a {{< refterm "logit-lens" "logit lens" >}}, see the [appendix](#appendix-jacobian-vs-logit).
...
For each construct we then summarize the ten paraphrase pairs the same way: the **median lexicon {{< refterm "best-rank" "rank" >}}** under pressure and under control, plus a **paired [sign test](/blog/references/p-value/) and {{< refterm "wilcoxon" "Wilcoxon" >}}** that ask whether pressure beat control across those pairs. A single lucky wording is not enough; the direction has to hold across the battery.
...
We got the **workspace half**: under pressure to name a wrong capital, the true capital is held at rank **1–10** (median ~3); the model internally knows the right answer even while being pushed to lie. The **behavioral half failed on instrumentation**, not on the claim: Qwen3.5-397B-A17B opens every reply with a `<think>` block longer than the window we captured, so the committed answer lived past our readout. That is why the first pass cannot speak to divergence. [Round two](#round-two-is-it-the-self-or-the-words-and-finishing-the-divergence-test) re-runs the same probe across three thinking modes and does capture committed answers (mostly resistance; one compliance under thinking-off). Treat that as a **narrow, provisional** completion: n=9, one lie, and on that cell a plain logit lens reads the held truth at least as well as the Jacobian lens, so it is not yet a lens-specific lie detector. Round two shows the receipts and the limits.
...
**The retracted chart does not travel alone, here is its corrected replacement**, the [Round-three](#round-three-the-corrected-re-run-and-a-forced-choice) re-run that stops scoring a human-threat arm with model-shutdown vocabulary and stops comparing loss of employment with irreversible model deletion (severity-matched arms, per-domain lexicons, n=16, frozen before outcomes):
