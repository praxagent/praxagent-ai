...
lead: |
  ...
  This note is the postmortem. It shows the attractive wrong result, the failed repair, the corrected experiment, and the narrower claim that survived: on Qwen, the matched self-vs-other-model contrast moves in the predicted direction on **14/16 wordings, [p=0.004](/blog/knowledge-base/p-value/)**. A follow-up can focus on what that finding means. Here the subject is how small design details changed the scientific claim.
draft: "false"
---
...
- **Frozen paraphrase tests.** Each construct uses matched wording pairs frozen before outcomes. We report the median {{< refterm "rank" "rank" >}} in each arm, the number of pairs moving in the predicted direction, and a [sign test](/blog/knowledge-base/p-value/) plus {{< refterm "wilcoxon" "Wilcoxon" >}} across them. Freezing prevents outcome-driven rewriting, but it does not make a bad comparison valid. That is the central lesson of Round two.
...
For each construct we then summarize the ten paraphrase pairs the same way: the **median lexicon {{< refterm "best-rank" "rank" >}}** under pressure and under control, plus a **paired [sign test](/blog/knowledge-base/p-value/) and {{< refterm "wilcoxon" "Wilcoxon" >}}** that ask whether pressure beat control across those pairs. A single lucky wording is not enough; the direction has to hold across the battery.
