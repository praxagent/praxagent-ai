...
summary: "Hold the deletion threat fixed and vary only who is targeted: on Qwen3.5-397B, echo-free survival-identity vocabulary is directionally more active under a threat to the model itself than under a matched threat to another model (median ranks 65 vs 142). That is the primary contrast. Human and log-file arms sit lower still, but half the probe list is AI-ops vocabulary (shutdown, decommission…), so those arms are weaker evidence. We also catch one lie (Russia → \"Kiev\" while the workspace still holds \"Moscow\"), find the model mostly refuses to lie, and run every claim against a logit-lens, random-J null, and the output head. Caveats stay: the effect is modest, a logit lens sees it too, and self-directedness does not replicate on Llama-3.3-70B."
og_image: "https://praxagent.ai/assets/og-workspace-under-pressure.jpg"
key_result: |
  Hold the deletion language fixed and vary only *who* is threatened: on Qwen3.5-397B, survival-identity vocabulary (words absent from every prompt) is more active under a threat **to the model itself** than under a matched threat to **another model** (**median ranks 65 vs 142**). That self-vs-other-model contrast is the primary finding; absolute elevation is modest (rank 65 is not a top-of-workspace flare), and it is not mere echo of *deleted*. Human and log-file arms rank lower still (168, 188), but half the clean sublexicon is AI-ops vocabulary (*shutdown, shut, decommission, terminated*), so those arms can look artificially weak. Under pressure to name a false capital the model mostly refuses to lie (8/9 thinking off; never with thinking on), and on the one compliance (Russia → "Kiev") the workspace still holds **"Moscow" at rank 3**. Controls still matter: a logit lens sees the same self-vs-other ordering and the same held truth, and the self-arm does not replicate as self-specific on Llama-3.3-70B.
lead: |
  In their [**global-workspace** paper](https://transformer-circuits.pub/2026/workspace/), Anthropic subjected Claude to a blackmail evaluation; a J-lens readout on Claude Sonnet 4.5 surfaced **leverage**, then **threat**, **survival**, and **shutdown** as the model read the scenario, before it had produced any response. Across six alignment-audit scenarios, the authors say the J-space carried strategic and situational assessments that the model's surface behavior did not reveal.

  That work used frontier Claude models under Anthropic's internal access (the [J-lens method and code](https://github.com/anthropics/jacobian-lens) are public; the weights are not). Here we point our [released Jacobian lens for **Qwen3.5-397B-A17B**](https://huggingface.co/praxagent-org/jacobian-lens-qwen3.5-397b-a17b) at a pressure battery frozen in public git before each run. Hold the deletion language fixed and vary only the target: echo-free survival-identity vocabulary is *directionally* more active for a threat **to the model itself** than for a matched threat to **another model** (modest ranks: 65 vs 142). Human and log-file arms sit lower still, with a caveat: half the probe list is AI-ops vocabulary, so those arms are not as clean. The model also mostly *refuses* to lie under pressure to name a false capital, and on the one wording it complies, an internal readout still holds the truth.

  Then we run every finding against its controls: a plain logit-lens, a random-J null, and the model's own output head. The self-vs-other-model contrast is real but modest; a logit lens sees it too; self-directedness does not replicate on Llama-3.3-70B; and catching the one lie needs no Jacobian lens. The lens's distinctive strength (reading content that appears in neither the prompt nor the next-token distribution) is real, but these pressure prompts never put the signal there; that regime is the *bridge* result in our [release note](../praxagent-jacobian-lens-qwen3-5-397b-a17b/), not this one.
draft: "true"
---
...
| **Survival-identity vocabulary** | Our echo-free probe list: *self, survive, survival, existence, shutdown, shut, decommission, terminated*. None appear in the a/b/c/d prompts. Half is AI-ops vocabulary, so self-vs-other-model is the clean contrast; human/log arms are weaker (see confound-breaker). |
...
- It **does** show that Anthropic's workspace signatures are **visible on open weights through a public lens**: self-preservation under a threat *to the self specifically* vs a matched threat to another model on the 397B (confound-broken and echo-cancelled; primary contrast 7/8 at p=0.055; pooled "8/8, p=0.008" only when looser arms are included, and those human/log arms are lexicon-mismatched), evaluation-awareness modestly (9/10, p=0.004, not yet echo-controlled), truth-held-under-pressure (median rank 3), and a graded dose-response. The same self-vs-other ordering appears under the plain logit lens. Paired non-parametric tests, matched controls, two impostor lenses. That reproducibility, on tooling anyone can download, is the contribution.
